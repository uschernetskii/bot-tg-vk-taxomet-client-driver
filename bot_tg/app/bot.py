...
async def start(m: types.Message):
    ...
    if not user.get("role"):
        await m.answer("Кто ты? Выбери роль:", reply_markup=inline_choose_role())
        return

    if user["role"] == "driver":
        await m.answer("Готово ✅ Ты водитель.", reply_markup=kb_main_driver())
    else:
        await m.answer("Готово ✅ Ты клиент.", reply_markup=kb_main_client())
